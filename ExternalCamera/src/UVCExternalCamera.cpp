...
uvc_frame_format
getUVCPixelFormat(VuforiaDriver::PixelFormat vuforiaFormat)
{
...
{
    VuforiaDriver::PixelFormat vuforiaFormat = VuforiaDriver::PixelFormat::UNKNOWN;

    switch (uvcFormat)
    {
        case UVC_FRAME_FORMAT_YUYV:
            vuforiaFormat = VuforiaDriver::PixelFormat::YUYV;
            break;
        default:
...
uvcCallbackFunc(uvc_frame_t* inFrame, void* user_ptr)
{
    ...
    VuforiaDriver::CameraCallback* vuforiaCallback = uvcExternalCamera->getCallback();
    if (vuforiaCallback)
    {
...
UVCExternalCamera::UVCExternalCamera(VuforiaDriver::PlatformData* platformData)
{
...
bool
UVCExternalCamera::start(VuforiaDriver::CameraMode cameraMode, VuforiaDriver::CameraCallback* cb)
{
    if (mDeviceHandle == nullptr)
    {
...
bool
UVCExternalCamera::getSupportedCameraMode(uint32_t index, VuforiaDriver::CameraMode* out)
{
...
bool
UVCExternalCamera::supportsExposureMode(VuforiaDriver::ExposureMode parameter)
{
    ...
    if (inputTerminal->bmControls & (1 << BIT_SHIFT_CT_AE_MODE_CONTROL))
    {
        ...
        switch (parameter)
        {
            ...
            case VuforiaDriver::ExposureMode::MANUAL:
                return supportedMode & EXPOSURE_MODE_MANUAL;
            case VuforiaDriver::ExposureMode::SHUTTER_PRIORITY:
...
UVCExternalCamera::getExposureMode()
{
    ...
    switch (mode)
    {
        case EXPOSURE_MODE_MANUAL:
            return VuforiaDriver::ExposureMode::MANUAL;
        case EXPOSURE_MODE_AUTO:
        case EXPOSURE_MODE_APERTURE_PRIORITY:
            // Both usb exposure mode AUTO and APERTURE PRIORITY are essentially continuous auto exposure
            // 1. AUTO - auto exposure time, auto iris
            // 2. APERTURE_PRIORITY - auto exposure time, manual iris
            return VuforiaDriver::ExposureMode::CONTINUOUS_AUTO;
        case EXPOSURE_MODE_SHUTTER_PRIORITY:
            return VuforiaDriver::ExposureMode::SHUTTER_PRIORITY;
        default:
...
bool
UVCExternalCamera::setExposureMode(VuforiaDriver::ExposureMode mode)
{
    uvc_error_t result;
...
bool
UVCExternalCamera::supportsFocusMode(VuforiaDriver::FocusMode parameter)
{
    const uvc_input_terminal_t* inputTerminal = uvc_get_input_terminals(mDeviceHandle);

    switch (parameter)
    {
        case VuforiaDriver::FocusMode::AUTO:
            // Unless otherwise stated auto focus is always continuous, not only for one exposure
            return false;
        case VuforiaDriver::FocusMode::CONTINUOUS_AUTO:
            return (inputTerminal->bmControls & (1 << BIT_SHIFT_CT_FOCUS_AUTO_CONTROL));
        case VuforiaDriver::FocusMode::MACRO:
        case VuforiaDriver::FocusMode::INFINITY_FOCUS:
#if SIMPLE_FOCUS_METHODS_IMPLEMENTED
            return (inputTerminal->bmControls & (1 << BIT_SHIFT_CT_FOCUS_SIMPLE_CONTROL));
#else
            return false;
#endif
        case VuforiaDriver::FocusMode::FIXED:
...
UVCExternalCamera::getFocusMode()
{
    ...
    {
        return VuforiaDriver::FocusMode::CONTINUOUS_AUTO;
    }
    ...
    if (result == UVC_SUCCESS)
    {
        switch (state)
        {
            case SIMPLE_FOCUS_MODE_MACRO:
                return VuforiaDriver::FocusMode::MACRO;
            case SIMPLE_FOCUS_MODE_SCENE:
                return VuforiaDriver::FocusMode::INFINITY_FOCUS;
            default:
                break;
        }
    }
    ...
    return VuforiaDriver::FocusMode::FIXED;
}

bool
UVCExternalCamera::setFocusMode(VuforiaDriver::FocusMode mode)
{
    uvc_error_t result;
...
UVCExternalCamera::getSupportedCameraModes()
{
    ...
    for (const uvc_format_desc_t* formatDesc = uvc_get_format_descs(mDeviceHandle); formatDesc != nullptr; formatDesc = formatDesc->next)
    {
        ...
        VuforiaDriver::PixelFormat pixelFormat = VuforiaDriver::PixelFormat::UNKNOWN;
        ...
        if (pixelFormat == VuforiaDriver::PixelFormat::UNKNOWN) {
...
UVCExternalCamera::getCalibrationValue(int vid, int pid, int width, int height)
{
    ...
    VuforiaDriver::CameraIntrinsics intrinsics; // default intrinsics with all zero values
