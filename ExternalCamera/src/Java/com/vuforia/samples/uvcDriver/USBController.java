...
public final class USBController
{
    ...
    public static void requestUSBPermission(Activity activity)
    {
        UsbManager usbManager = (UsbManager)activity.getSystemService(Context.USB_SERVICE);
        HashMap<String, UsbDevice> usbDeviceHashMap = usbManager.getDeviceList();

        UsbDevice[] usbDeviceList = new UsbDevice[usbDeviceHashMap.size()];
        usbDeviceList = usbDeviceHashMap.values().toArray(usbDeviceList);

        final HashMap<UsbDevice, CountDownLatch> permissionLatchMap =
            new HashMap<UsbDevice, CountDownLatch>(usbDeviceList.length);
        ...
        BroadcastReceiver broadcastReceiver = new BroadcastReceiver()
        {
            ...
            public void onReceive(Context context, Intent intent)
            {
                if (intent.getAction().equals(REQUEST_USB_PERMISSION))
                {
                    ...
                    if (device != null && permissionLatchMap.get(device) != null)
                    {
                        // We got the permission result for this device, countdown the latch 
                        permissionLatchMap.get(device).countDown();
                    }
                }
            }
        };
        ...
        // Broadcast request
        for (int idx = 0; idx < usbDeviceList.length; idx++)
        {
            UsbDevice device = usbDeviceList[idx];
            if (!usbManager.hasPermission(device))
            {
                ...
                permissionLatchMap.put(device, new CountDownLatch(1));
            }
        }
        ...
        {
            UsbDevice device = usbDeviceList[idx];
            if (permissionLatchMap.get(device) != null)
            {	
                try	
                {	
                    permissionLatchMap.get(device).await();
                }	
                catch (InterruptedException ex)	
                {	
                    Log.e(MODULE_TAG, "Error while awaiting permission latch for usb device at index " + idx, ex);	
                    return;	
                }	
            }    
        }
